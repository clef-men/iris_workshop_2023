...
\begin{tikzpicture}
	...
	\node[left] at (-1, 0.5) {\small Processor \ding{172}} ;
	\draw[step=1cm, gray] (0, 0) grid ++(\lenI, 1) ;
	\draw[step=1cm, gray] (\lenI, 0) grid ++(\lenxI, 1) ;
	\fill[blue, opacity=0.2] (\lenI, 0) rectangle ++(\lenxI, 1) ;
	\draw[decorate, decoration={brace, amplitude=5pt}] (\lenI, -0.5) -- ++(- \lenI, 0) node [midway, below, yshift=-2mm] {\footnotesize work-stealing deque} ;
	\draw[->] (- \hsep, \vsep) -- ++(2 * \hsep - 1, 0) node[midway, below] {\scriptsize\texttt{pop}} ;
	\draw[->] (\hsep - 1, 1 - \vsep) -- ++(1 - 2 * \hsep, 0) node[midway, above] {\scriptsize\texttt{push}} ;
	\foreach \x in {1, ..., \lenI} {
		\node at (\x - 0.5, 0.5) {\scriptsize task} ;
	}
	
	\node[left] at (-1, \sep + 1.5) {\small Processor \ding{173}} ;
	...
	\draw[->] (- \hsep, \sep + 1 + \vsep) -- ++(2 * \hsep - 1, 0) node[midway, below] {\scriptsize\texttt{pop}} ;
	\draw[->] (\hsep - 1, \sep + 2 - \vsep) -- ++(1 - 2 * \hsep, 0) node[midway, above] {\scriptsize\texttt{push}} ;
	\foreach \x in {1, ..., \lenII} {
		\node at (\x - 0.5, \sep + 1.5) {\scriptsize task} ;
	}
	\node at (\lenII + \lenxII - 0.5, \sep + 1.5) {\scriptsize task} ;
	
	\node[left] at (-1, 2 * \sep + 2.5) {\small Processor \ding{174}} ;
	...
	\draw[->] (- \hsep, 2 * \sep + 2 + \vsep) -- ++(2 * \hsep - 1, 0) node[midway, below] {\scriptsize\texttt{pop}} ;
	\draw[->] (\hsep - 1, 2 * \sep + 3 - \vsep) -- ++(1 - 2 * \hsep, 0) node[midway, above] {\scriptsize\texttt{push}} ;
	
	\draw[->, to path={|- node[above] {\scriptsize\texttt{steal}} (\tikztotarget)}] (\lenII + \lenxII - 0.5, \sep + 2) to (\lenIII + \lenxIII, 2 * \sep + 2.5) ;
\end{tikzpicture}
\end{frame}
