...
\only<2>{
	\tikz[remember picture] \node[align=left] (inv_2) {
		imposes a protocol (using an Iris invariant)
	} ;
...
\only<3>{
	\tikz[remember picture] \node[align=left] (model_2) {
		asserts the list of values that the structure (logically) contains
	} ;
	\begin{tikzpicture}[remember picture, overlay]
...
\only<5>{
	\tikz[remember picture] \node[align=left] (model_3) {
		some value $v$ is atomically pushed at the owner's end
	} ;
	\begin{tikzpicture}[remember picture, overlay]
