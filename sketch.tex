...
\begin{tikzpicture}[scale=1.15]
	...
	\only<1->{
		\draw[step=1cm, gray, thin] (0, 0) grid (\hist + \model + \priv, 1) ;
	...
	\only<4->{
		\draw[very thick] (\hist + \model, 0) -- (\hist + \model, 1) node[label=above:back] {} ;
	}
	
	\only<5->{
		\fill[red, opacity=0.2] (\hist + \model, 0) rectangle (\hist + \model + \priv, 1) ;
		\draw[decorate, decoration={brace, amplitude=5pt}] (\hist + \model + \priv, -0.2) -- (\hist + \model, -0.2) node [midway, yshift=-5mm] {priv} ;
	}
	
	\only<6->{
		\fill[green, opacity=0.2] (\hist, 0) rectangle (\hist + \model, 1) ;
		\draw[decorate, decoration={brace, amplitude=5pt}] (\hist + \model, -0.2) -- (\hist, -0.2) node [midway, yshift=-5mm] {model} ;
	}
	\only<7->{
	...
	\only<10->{
		...
		\draw[thick, ->, teal] (\hist + \model, 0.5) -- (\hist + \model - 1, 0.5) node[label=below right:{\footnotesize\texttt{pop}}] {} ;
		\draw[thick, ->, red] (\hist + \model, 0.5) -- (\hist + \model + 1, 0.5) node[label=below left:{\footnotesize\texttt{push}}] {} ;
	}
\end{tikzpicture}
\vfill
\begin{itemize}
	...
	\only<1->{
	   ...
	   \node[align=center] (s2) [right=\width of s1] {
			...
			\begin{tikzpicture}[scale=0.5]
				...
				\fill[green, opacity=0.2] (\hist, 0) rectangle (\hist + \model, 1) ;
				\draw[very thick] (\hist + \model, 1) -- (\hist + \model, 0) node[yshift=2mm, label=below:{\tiny back}] {} ;
				\fill[red, opacity=0.2] (\hist + \model, 0) rectangle (\hist + \model + \priv, 1) ;
			\end{tikzpicture}
	   } ;
   }
