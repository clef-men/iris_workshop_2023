...
\begin{tikzpicture}[scale=1.15]
	...
	\only<9->{
		\fill[blue, opacity=0.2] (\hist, 0) rectangle ++(\histext, 1) ;
		\draw[decorate, decoration={brace, amplitude=5pt}] (\hist + \histext, - 1) -- ++(- \hist - \histext, 0) node [midway, yshift=-5mm] {hist\textsubscript{+}} ;
	}
   ...
   \only<3->{
	   \node[align=center] (s3) [below=\height of s2] {
			...
			\begin{tikzpicture}[scale=0.5]
				...
				\fill[blue, opacity=0.2] (0, 0) rectangle (\hist + \histext, 1) ;
				\draw[very thick] (\hist, 1) -- ++(0, - 1) node[yshift=2mm, label=below:{\tiny front = back}] {} ;
				\fill[red, opacity=0.2] (\hist + \histext, 0) rectangle ++(\priv - \histext, 1) ;
			\end{tikzpicture}
	   } ;
	   \node[align=center] (s4) [below=\height of s1] {
	   	\ding{175} super empty \\
			\begin{tikzpicture}[scale=0.5]
				...
				\draw[very thick] (\hist + \histext, 1) -- ++(0, - 1) node[xshift=2mm, yshift=2mm, label=below:{\tiny front}] {} ;
				\fill[red, opacity=0.2] (\hist + \histext, 0) rectangle ++(\priv - \histext, 1) ;
			\end{tikzpicture}
	   } ;
   }
